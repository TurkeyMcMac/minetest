...
> struct_serialize(std::string &buf, const T &val);
...
> struct_serialize(std::string &buf, const T &val)
{
	buf += std::string((const char *)&val, sizeof(T));
}
...
> struct_serialize_tuple(std::string &buf, const T &val)
{
}
...
template<typename T>
std::string struct_serialize(const T &val)
{
	std::string buf;
	struct_serialize(buf, val);
	return buf;
}
